...
compile_harness() {
  ...
  ${CC:-cc} -O3 -std=c11 \
    -I"$ROOT/include" \
    "$ROOT/bench/bench_ns_path.c" \
    "$build_dir/libaudio_pipeline.a" -lm -o "$output"
}

compile_harness "$TMP/base/build-ns-perf" "$TMP/base-ns-bench"
compile_harness "$TMP/head-build" "$TMP/head-ns-bench"

extract_us() {
  "$1" "$FRAMES" "$2" "$3" | sed -n 's/.* us_per_frame=\([0-9.]*\).*/\1/p'
}
...
run_case() {
  ...
  while [ "$i" -lt "$REPS" ]; do
    if [ $((i % 2)) -eq 0 ]; then
      base_us=$(extract_us "$TMP/base-ns-bench" "$rate" "$res")
      head_us=$(extract_us "$TMP/head-ns-bench" "$rate" "$res")
    else
      head_us=$(extract_us "$TMP/head-ns-bench" "$rate" "$res")
      base_us=$(extract_us "$TMP/base-ns-bench" "$rate" "$res")
    fi
  ...
  if awk -v d="$delta" 'BEGIN { exit !(d > 10.0) }'; then
    echo "NS path same-runner regression exceeds 10% rate=$rate res=$res" >&2
    exit 3
  fi
}
