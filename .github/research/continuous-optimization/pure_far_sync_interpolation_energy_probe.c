...
static float clamp_frac(float x) {
    return x < -0.999f ? -0.999f : (x > 0.999f ? 0.999f : x);
}

static float render_absolute(const ap_sync_state_t *s, int64_t index) {
...
int main(int argc, char **argv) {
    ...
    while (fread(mic, sizeof(int16_t), frame, fm) == frame) {
        ...
        float interpolated_reference_energy = 1.0e-12f;
        float nearest_reference_energy = 1.0e-12f;
        float reconstruction_max_abs_error = 0.0f;
        float metric_mic_energy;
        float smoothed_ratio;
        float instant_ratio;
        float nearest_mic_reference_ratio;
        float interpolation_energy_ratio;
        float interpolation_energy_delta_db;
        float dt_ratio;
        float frac;
        int64_t start;
        int reference_start_valid;
        int dt_on_evidence;
        ...
        start = (int64_t)sync->render_total - (int64_t)pipeline->internal_frame -
                (int64_t)sync->delay_samples;
        reference_start_valid = start >= 0;
        frac = clamp_frac(sync->drift_credit);
        for (i = 0u; i < pipeline->internal_frame; ++i) {
            const int64_t at = start + (int64_t)i;
            const float nearest = render_absolute(sync, at);
            const float current = nearest;
            float reconstructed;
            float error;
            if (frac >= 0.0f) {
                const float previous = render_absolute(sync, at - 1);
                reconstructed = current + frac * (previous - current);
            } else {
                const float next = render_absolute(sync, at + 1);
                reconstructed = current + (-frac) * (next - current);
            }
            nearest_reference_energy += nearest * nearest;
            interpolated_reference_energy += pipeline->reference[i] * pipeline->reference[i];
            error = fabsf(reconstructed - pipeline->reference[i]);
            if (error > reconstruction_max_abs_error)
                reconstruction_max_abs_error = error;
        }
        nearest_reference_energy /= pipeline->internal_frame;
        interpolated_reference_energy /= pipeline->internal_frame;
        ...
        nearest_mic_reference_ratio = metric_mic_energy /
                                      (nearest_reference_energy + 1.0e-12f);
        interpolation_energy_ratio = interpolated_reference_energy /
                                     (nearest_reference_energy + 1.0e-12f);
        interpolation_energy_delta_db = 10.0f * log10f(
            (interpolated_reference_energy + 1.0e-12f) /
            (nearest_reference_energy + 1.0e-12f));
        ...
        if (fprintf(fo,
                    ...
                    "\"delay_error_samples\":%d,\"delay_samples\":%u,"
                    ...
                    "\"interpolation_energy_ratio\":%.9g,"
                    "\"interpolation_energy_delta_db\":%.9g,"
                    "\"sync_reconstruction_max_abs_error\":%.9g,"
                    ...
                    (double)interpolation_energy_ratio,
                    (double)interpolation_energy_delta_db,
