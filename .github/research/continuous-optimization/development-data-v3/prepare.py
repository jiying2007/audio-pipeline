#!/usr/bin/env python3
"""Materialize SHA-256 pinned research-development-v3 audio on GitHub-hosted runners."""
...
NEW_VETTED_ROLES = {
    "noise-living": "DLIVING_16k.zip",
    "noise-office": "OOFFICE_16k.zip",
    "noise-cafeteria": "PCAFETER_16k.zip",
    "noise-bus": "TBUS_16k.zip",
    "noise-field": "NFIELD_16k.zip",
}
...
def validate_lock(lock: dict) -> None:
    ...
    if set(lock) != {"schema_version", "catalog_id", "source_vetting_receipt", "archives"}:
        raise ValueError("research development-v3 lock fields invalid")
    if not isinstance(lock["source_vetting_receipt"], str) or not lock["source_vetting_receipt"]:
        raise ValueError("source_vetting_receipt required")
    if not isinstance(archives, list) or len(archives) != len(EXPECTED_ROLES):
        raise ValueError("research development-v3 lock archive count invalid")
    roles, ids = set(), set()
...
def validate_receipt_binding(lock: dict, receipt: dict) -> None:
    if (
        receipt.get("candidate_id") != "public-development-demand-domain-expansion-v3"
        or receipt.get("authority") != "SOURCE_VETTING_ONLY"
        or receipt.get("selection_authorized") is not False
    ):
        raise ValueError("source-vetting receipt authority drift")
    files = receipt.get("files")
    if not isinstance(files, list) or len(files) != len(NEW_VETTED_ROLES):
        raise ValueError("source-vetting receipt file count drift")
    vetted = {item.get("name"): item for item in files}
    if len(vetted) != len(files):
        raise ValueError("duplicate source-vetting receipt filename")
    for role, filename in NEW_VETTED_ROLES.items():
        item = by_role(lock, role)
        source = vetted.get(filename)
        if source is None:
            raise ValueError(f"missing source-vetting receipt file: {filename}")
        if int(source.get("bytes", -1)) != int(item["bytes"]):
            raise ValueError(f"source-vetting byte-size mismatch for role={role}")
        if source.get("sha256") != item["sha256"]:
            raise ValueError(f"source-vetting SHA256 mismatch for role={role}")


def materialize(lock_path: Path, root: Path, output: Path) -> dict:
    lock = load_json(lock_path)
    validate_lock(lock)
    receipt_path = Path(lock["source_vetting_receipt"])
    if receipt_path.is_absolute() or ".." in receipt_path.parts:
        raise ValueError("source_vetting_receipt must stay in repository")
    receipt = load_json(receipt_path)
    validate_receipt_binding(lock, receipt)
    root = root.resolve()
    ...
    result = {
        ...
        "source_vetting_receipt_sha256": sha256_file(receipt_path),
        "root": str(root),
        "archives": evidence,
...
def self_test() -> None:
    fixture = {
        ...
        "source_vetting_receipt": "fixture-receipt.json",
        "archives": [
    ...
    fixture_receipt = {
        ...
        "files": [
            {
                "name": filename,
                "bytes": by_role(fixture, role)["bytes"],
                "sha256": by_role(fixture, role)["sha256"],
            }
            for role, filename in NEW_VETTED_ROLES.items()
        ],
    }
    validate_receipt_binding(fixture, fixture_receipt)
    bad_receipt = json.loads(json.dumps(fixture_receipt))
    bad_receipt["files"][0]["sha256"] = "0" * 64
    try:
        validate_receipt_binding(fixture, bad_receipt)
    except ValueError:
        pass
    else:
        raise AssertionError("receipt SHA drift accepted")
    for bad in ("../x", "/tmp/x"):
        try:
            _safe_member(bad)
        except ValueError:
    ...
    assert len(EXPECTED_ROLES) == 9 and len(NEW_VETTED_ROLES) == 5
    print("research development-v3 materializer self-test: OK")
...
def main() -> int:
    ...
    if a.check:
        if a.lock is None:
            p.error("--check requires --lock")
        lock = load_json(a.lock)
        validate_lock(lock)
        receipt_path = Path(lock["source_vetting_receipt"])
        validate_receipt_binding(lock, load_json(receipt_path))
        print(json.dumps({"result": "PASS", "catalog_id": lock["catalog_id"],
                          "archives": len(lock["archives"]),
                          "vetted_new_noise_domains": len(NEW_VETTED_ROLES)}, sort_keys=True))
        return 0
