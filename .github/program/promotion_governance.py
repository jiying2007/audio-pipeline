...
FORBIDDEN_PREFIXES = (
    "src/", "include/", "validation/policies/", "validation/datasets.lock.json",
    "validation/hosted_real.datasets.lock.json", "validation/hosted_aec.datasets.lock.json",
    "validation/tools/", "ci/resource-baseline.json", "certification/",
)
...
def validate_policy(policy: dict) -> None:
    ...
    for group in groups:
        ...
        allowed, prohibited = group["allowed_roles"], group["prohibited_roles"]
        ...
        require(set(allowed) <= valid_roles and set(prohibited) <= valid_roles and
                not (set(allowed) & set(prohibited)), "source role contract")
    ...
    require(policy["confirmation_policy"] == {
    ...
    for budget in budgets:
        ...
        for key in ("search_rounds", "candidate_variants", "confirmation_sets"):
            item = budget[key]
            require(set(item) == {"limit", "consumed"} and type(item["limit"]) is int and
                    type(item["consumed"]) is int and 0 <= item["consumed"] <= item["limit"],
                    "budget counter bounds")
...
def validate_gate_evidence(policy: dict, record: dict, stage: str = "before_merge") -> None:
    ...
    require(len(ids) == len(set(ids)), "duplicate attempt id")
    required_map = {
    ...
    latest = attempts[-1]
    require(set(latest) == {"attempt_id", "result", "gates"} and latest["result"] in {"PASS", "FAIL", "BLOCKED"},
            "attempt contract")
    for gate in required_map[stage]:
        require(gate in latest["gates"], f"missing required gate: {gate}")
        value = latest["gates"][gate]
...
def validate_tuning_workflow(workflow_text: str) -> None:
    ...
    for forbidden in ("unseen motion", "fresh call", "independent multi-seed"):
        require(forbidden not in lower, f"fixed data mislabeled as independent: {forbidden}")
    require("regression/replay" in lower, "fixed tuning partitions must declare regression/replay authority")
    require("effective_replay_authority" in workflow_text and "non-shipping-regression-replay" in workflow_text,
            "workflow finalizer must normalize fixed replay authority")
...
def validate_contract(contract: dict) -> None:
    ...
    require(exact_sha(contract["base_sha"]) and contract["policy"] == str(POLICY_PATH), "P001 baseline/policy")
...
def self_test() -> None:
    ...
    gates = {name: {"sha": sha, "status": "completed", "conclusion": "success"}
    ...
    bad_policy["source_groups"][0]["allowed_roles"].append("confirmation")
    bad_policy["source_groups"][0]["prohibited_roles"].remove("confirmation")
    for bad in (bad_policy, {**policy, "principles": {**policy["principles"], "auto_promote": True}}):
        try:
            validate_policy(bad)
        except ValueError:
            pass
    ...
    old["attempts"][-1]["gates"]["base-identity"]["sha"] = "0" * 40
    cases.append(old)
    skipped = copy.deepcopy(record)
    skipped["attempts"][-1]["gates"]["shadow-protection"].update(conclusion="skipped")
    cases.append(skipped)
    concealed = copy.deepcopy(record)
    concealed["attempts"] = concealed["attempts"][1:]
    cases.append(concealed)
    for bad in cases:
        try:
    ...
    good = """on:\n  pull_request:\n    paths:\n      - 'validation/tools/aec_motion_tuning.py'\n      - '.github/workflows/aec-motion-tuning.yml'\n  schedule:\n    - cron: '0 0 * * *'\n# regression/replay\neffective_replay_authority = 'non-shipping-regression-replay'\n"""
    validate_tuning_workflow(good)
    bad_trigger = good.replace("      - 'validation/tools/aec_motion_tuning.py'\n",
                               "      - 'validation/tools/build_aec_motion_corpus.py'\n      - 'validation/tools/aec_motion_tuning.py'\n")
    try:
        validate_tuning_workflow(bad_trigger)
    except ValueError:
        pass
    else:
        raise AssertionError("measurement-only tuning trigger accepted")
    print("P001 governance self-test: source/gate/history/budget/trigger negative contracts OK")
...
def run(contract_path: Path, output: Path) -> int:
    ...
    validate_tuning_workflow(workflow_text)
    result = {
        "schema_version": 1,
        "iteration_id": "P001",
        ...
        "bindings": {
            ...
            "governance_validator_sha256": digest(Path(__file__)),
        ...
        "checks": {
            ...
            "fixed_data_effective_authority_is_regression_replay": True,
            "i003_still_planned": True,
        },
    }
    write_json(output / "governance-result.json", result)
    print(json.dumps({"iteration_id": "P001", "result": "COMPLETE", "decision": result["decision"],
                      "head_sha": head_sha}, sort_keys=True))
    return 0
