...
def run_probe(probe: Path, raw_pcm: Path) -> list[dict[str, float | int]]:
    ...
    if not rows:
        raise ValueError("I012 v2 probe produced no rows")
    return rows
...
def run_case(probe: Path, corpus_path: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    if mic is None or labels_path is None:
        raise ValueError(f"I012 v2 case missing mic/labels: {case.get('case_id')}")
    if int(case.get("sample_rate_hz", 0)) != 16000:
        raise ValueError("I012 v2 requires 16 kHz")
    if int(case.get("mic_channels", 0)) != 1:
        raise ValueError("I012 v2 requires mono input")

    with tempfile.TemporaryDirectory(prefix="ap-i012-v2-stage-") as tmp:
        work = Path(tmp)
    ...
    if count <= WARMUP_FRAMES:
        raise ValueError("I012 v2 case too short after warmup")
    return {
...
def summarize(seed_cases: dict[int, list[dict[str, Any]]]) -> dict[str, Any]:
        ...
        for case in cases:
            ...
            for index, (label, row) in enumerate(zip(case["labels"], case["rows"])):
                ...
                if not all(math.isfinite(value) for value in (
                    ...
                    raise ValueError(
                        f"non-finite I012 v2 metric seed={seed} "
                        f"case={case['case_id']} frame={index}"
                    )
                if int(row["nfft"]) != 512:
                    raise ValueError("I012 v2 FFT geometry drifted")
                if (
                    int(row["band_lo_exclusive"]) != 8
                    or int(row["band_hi_exclusive"]) != 224
                    or int(row["band_bins"]) != 215
                ):
                    raise ValueError("I012 v2 speech-band geometry drifted")
    ...
    if not speech_spectral or not noise_spectral:
        raise ValueError("I012 v2 did not exercise speech and noise labels")
    ...
    return {
        ...
        "spectral_snr_median_separation_db": (
            statistics.median(speech_spectral)
            - statistics.median(noise_spectral)
        ),
        "per_seed": per_seed,
    }
...
def decide(
    ...
    min_seed_auc = min(
        float(item["spectral_snr_auc"]) for item in summary["per_seed"]
    )
    info_checks = [
        (
    ...
    if input_violations:
        decision = "NS_SPECTRAL_POST_SNR_INPUT_INVALID_REVIEW_REQUIRED"
    elif info_violations:
        decision = "NS_SPECTRAL_POST_SNR_INFORMATION_GAIN_NOT_SUPPORTED_REVIEW_REQUIRED"
    else:
        decision = "NS_SPECTRAL_POST_SNR_INFORMATION_GAIN_SUPPORTED_REVIEW_REQUIRED"
...
def evaluate(
    ...
    if len(corpora) != len(seeds):
        raise ValueError("I012 v2 corpus count must match preregistered seeds")
    if contract["budget"] != {
        "candidate_limit": 0,
        "confirmation_limit": 0,
        "diagnostic_execution_limit": 1,
    }:
        raise ValueError("I012 v2 budget drift")
    if (
        ...
        raise ValueError("I012 v2 authority boundary drift")
    ...
    for corpus_path in corpora:
        ...
        if set(selected) != REQUIRED_CASES:
            raise ValueError(f"I012 v2 required NS cases missing for seed {seed}")
        seed_cases[seed] = [
    ...
    if actual_seeds != seeds:
        raise ValueError(f"I012 v2 seed drift: {actual_seeds} != {seeds}")

    summary = summarize(seed_cases)
    decision, input_violations, info_violations = decide(
    ...
    result = {
        ...
        "interpretation": {
            ...
            "note": (
                ...
                "This investigation cannot select a production feature mapping, adaptive "
                "state rule, source candidate, or threshold."
            ),
        },
...
def self_test() -> None:
    ...
    decision, invalid, violations = decide(summary, gates)
    ...
    summary["max_floor_spectral_snr_delta_db"] = 0.1
    decision, invalid, _ = decide(summary, gates)
    assert decision == "NS_SPECTRAL_POST_SNR_INPUT_INVALID_REVIEW_REQUIRED"
    assert invalid[0]["gate"] == "floor_spectral_snr_invariance"
    print("I012 v2 spectral post-SNR evaluator self-test: OK")
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "information_violations": result["information_violations"],
    }, sort_keys=True))
    return 2 if result["decision"] == (
        "NS_SPECTRAL_POST_SNR_INPUT_INVALID_REVIEW_REQUIRED"
    ) else 0
