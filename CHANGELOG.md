...
- Preserve `neighbor_count` as the evaluated-neighbor count for compatibility and add `evaluated_neighbor_count` explicitly. This prevents one-at-a-time searches from making a partially observed local neighborhood look complete when a theoretical one-step combination was never generated by the bounded search.
- Extend deterministic self-tests across complete Cartesian coverage and incomplete one-at-a-time coverage. Candidate generation, score-first gate-compliant selection, DSP runtime defaults, shipping baseline and HIL/Product Certification authority are unchanged.
...
- Extend the tuning iteration engine so a gate applies to the subset of cases that define its metric, because metrics are intrinsically per-scenario (near-end SI-SDR covers 15 of 27 smoke cases, noise attenuation 12, ERLE 11). Coverage that differs between baseline and candidate remains a hard `case_metric_coverage_mismatch` violation, and a gate matching no case fails closed as `case_gate_not_applicable`. Add `minimum_delta`/`maximum_delta` bounds, the `max` per-case delta statistic for minimize-direction metrics such as VAD false positive rate and output clip fraction, and `min_cases` so a gate cannot pass vacuously. Extend the tuning self-test to cover partial applicability, upper bounds and fail-closed gate configuration.
- Declare `objective.case_delta_gates` in the tuning search-space schema, which previously omitted the field already used by the AEC-motion and NS stage spaces.
- Extend the `call-v1` grid past its truncated edges: `ns_floor` down to 0.05 and `agc_target_dbfs` up to -16.0, with `max_candidates` raised to 32. The wide search had been selecting its winner at the very edge of both grids, and a directed probe showed `ns_floor` has an interior peak at 0.07 that the old grid started to the right of.
- Add `search-spaces/call-interaction-v1.json`, a bounded Cartesian research run over `ns_floor` x `agc_target_dbfs`. One-at-a-time cannot express combinations, and these two controls stack: the best single-control change scores 1.5458 while the best gated combination reaches 3.0721. It also shows the gates binding at the right place, rejecting the highest-scoring grid point (3.1266) on a -0.7703 dB worst-case noise attenuation delta and selecting the best compliant point instead. `aec_mu` is deliberately excluded and the reason is recorded in `validation/tuning/README.md`: every probed combination raising it above the baseline violates the near-end SI-SDR gate, because the per-control costs of roughly 0.27 dB each are individually inside the 0.75 dB tolerance but exceed it when stacked.
- Keep the aggregate objective, scores, decision authority and product DSP unchanged: under the PR-smoke space the selected development candidate, its identifier and its score are bit-identical before and after this change. The gates reject candidates the aggregate objective had scored as acceptable or merely weak (`ns_floor=0.14` at +0.0358, `0.16` at +0.0842 and `0.18` at +0.6332, with worst-case noise attenuation deltas of -0.8119, -1.5905 and -2.3299 dB; `aec_mu=0.20` with a VAD false positive rate excursion of +0.0615).
- Record the held-out behaviour of the selected interaction candidate from the replay artifacts the engine already persists: `ns_floor=0.06` with `agc_target_dbfs=-15.0` scores 3.0721 on development and, recomputed with the same engine, +6.2844 on validation and +3.3241 on shadow, with zero gate violations on either partition. Its worst-case per-case deltas stay inside the 0.75 dB tolerance on both held-out partitions but only just (near-end SI-SDR -0.5892 and -0.6079 dB, noise-only attenuation -0.7167 and -0.6974 dB), so this remains a discovery signal: promotion still requires the blind holdout, resource, HIL and product certification workflow.
- Document two findings from probing past the edge of the interaction grid: `agc_target_dbfs` stops at -15.0 because that is where the paired gates bind, not because the grid is truncated (-14.0 and -13.0 score higher at 3.1618 and 3.1757 but violate noise attenuation at -0.9851 and -1.2595 dB), and the selected candidate rides only 0.013 dB inside that gate while stepping back to -16.0 gives up about 5% of the score for roughly eighteen times the margin and better near-end SI-SDR on all three partitions. The engine keeps selecting by score; selecting by margin is a policy decision and is not changed here. The development ranking is also not a reliable ordering at this granularity: `ns_floor=0.07` with `agc_target_dbfs=-16.0` scores 2.9663 on development against 2.9143 for `ns_floor=0.06`, yet only +5.5518 and +2.7915 on the held-out partitions against +6.1094 and +3.1605, so differences of a few hundredths of a development score must not be read as ordering evidence.
...
- Remove duplicate lab dependency version literals from the documentation consistency checker: each `lab/requirements-*.txt` file remains its version authority, while CI still requires the expected package to be one exact `name==X.Y.Z` SemVer pin.
