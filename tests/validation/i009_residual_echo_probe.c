...
int main(int argc, char **argv) {
    ...
    FILE *fr;
    unsigned frame = 0u;
    float smoothed_previous_echo = 0.0f;
    float smoothed_previous_residual = 0.0f;
    ...
    while (fread(mic, sizeof(mic[0]), FRAME, fm) == FRAME) {
        ...
        const float previous_residual_energy = energy_f32(pipeline->aec_out, pipeline->internal_frame);
        ...
        float residual_echo_smoothed_ratio;
        float residual_echo_instant_ratio;

        if (got < FRAME)
            memset(render + got, 0, (FRAME - got) * sizeof(render[0]));

        smoothed_previous_echo = smooth_energy(smoothed_previous_echo, previous_echo_energy);
        smoothed_previous_residual = smooth_energy(smoothed_previous_residual, previous_residual_energy);
        ...
        residual_echo_smoothed_ratio = smoothed_previous_residual /
                                       (smoothed_previous_echo + 1.0e-12f);
        residual_echo_instant_ratio = previous_residual_energy /
                                      (previous_echo_energy + 1.0e-12f);
        ...
        printf("{\"frame\":%u,\"far_end_active\":%u,\"double_talk_active\":%u,"
               ...
               "\"previous_echo_energy\":%.9g,\"previous_residual_energy\":%.9g,"
               "\"smoothed_previous_echo_energy\":%.9g,"
               "\"smoothed_previous_residual_energy\":%.9g,"
               "\"raw_smoothed_ratio\":%.9g,\"raw_instant_ratio\":%.9g,"
               "\"residual_echo_smoothed_ratio\":%.9g,"
               "\"residual_echo_instant_ratio\":%.9g}\n",
               frame,
               ...
               (double)previous_residual_energy,
               (double)smoothed_previous_echo,
               (double)smoothed_previous_residual,
               (double)raw_smoothed_ratio,
               (double)raw_instant_ratio,
               (double)residual_echo_smoothed_ratio,
               (double)residual_echo_instant_ratio);
        frame++;
    }
