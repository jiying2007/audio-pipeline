#!/usr/bin/env python3
"""Candidate-zero pure-far HPF energy asymmetry diagnostic."""
...
def validate_manifest(manifest: dict) -> None:
    ...
    require(manifest.get("investigation_id") == "pure-far-hpf-energy-asymmetry-v1", "manifest identity")
...
def summarize_hpf(rows: list[dict]) -> dict:
    far = [r for r in rows if int(r["far_end_active"]) != 0]
    require(far, "far-active frames required")
    pre_ratio = [float(r["shadow_raw_mic_energy"]) / (float(r["direct_reference_energy"]) + 1.0e-12)
                 for r in far]
    post_ratio = [float(r["shadow_hpf_mic_energy"]) / (float(r["direct_reference_energy"]) + 1.0e-12)
                  for r in far]
    return {
        "far_frame_count": len(far),
        "hpf_energy_ratio": distribution([r["shadow_hpf_energy_ratio"] for r in far]),
...
def run_case(case: dict, source_root: Path, probe: Path, helper, manifest: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="pure-far-hpf-asym-") as tmp:
        root = Path(tmp)
    ...
    return {
        ...
        "activity": activity,
        "hpf": summarize_hpf(rows),
    }


def aggregate(cases: list[dict]) -> dict:
    dtd = [c["activity"]["double_talk_fraction"] for c in cases]
    delta_db = [c["hpf"]["hpf_energy_delta_db"]["median"] for c in cases]
    ...
    return {
        ...
        "distributions": {
            ...
            "median_hpf_energy_delta_db": distribution(delta_db),
        ...
        "spearman_associations": {
            ...
            "dtd_vs_hpf_energy_ratio": spearman(dtd, energy_ratio),
            "dtd_vs_pre_hpf_mic_reference_ratio": spearman(dtd, pre_ratio),
            "dtd_vs_post_hpf_mic_reference_ratio": spearman(dtd, post_ratio),
            "pre_vs_post_mic_reference_ratio": spearman(pre_ratio, post_ratio),
        },
    }
...
def self_test() -> None:
    ...
    print("pure-far HPF energy asymmetry analyzer self-test: OK")
...
def main() -> int:
    ...
    write_json(args.output / "pure-far-hpf-energy-asymmetry-result.json", payload)
    with (args.output / "summary.md").open("w", encoding="utf-8") as fh:
        fh.write("## Pure-far HPF energy asymmetry v1\n\n")
