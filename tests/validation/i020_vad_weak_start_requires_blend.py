...
def evaluate_case(probe: Path, corpus_path: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ap-i020-") as tmp:
    ...
    blend_mismatch = sum(
        int(row["shipping_blend_applied"]) != int(row["candidate_blend_applied"])
        for row in rows
    )
    if blend_mismatch:
        raise ValueError("state-only candidate changed blend application")

    blocked = {"speech": 0, "noise": 0}
    for row, label in zip(rows, labels):
        if int(row["candidate_weak_start_blocked"]):
            blocked["speech" if label else "noise"] += 1
    ...
    return {
        ...
        "candidate_weak_start_blocked_frames": {
            **blocked,
            "total": blocked["speech"] + blocked["noise"],
        },
    }
...
def merge(parts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    blocked_speech = sum(
        int(part["candidate_weak_start_blocked_frames"]["speech"]) for part in parts
    )
    blocked_noise = sum(
        int(part["candidate_weak_start_blocked_frames"]["noise"]) for part in parts
    )
    out["candidate_weak_start_blocked_frames"] = {
        "speech": blocked_speech,
        "noise": blocked_noise,
        "total": blocked_speech + blocked_noise,
    }
...
def evaluate(
    ...
    if contract["investigation_id"] != "i020-vad-weak-start-requires-blend-v1":
    ...
    nonstat = summaries["stage-ns-nonstationary"]
    delta = nonstat["candidate_vs_shipping"]
    if int(delta["noise_active_reduction_frames"]) < int(
        gates["min_nonstationary_noise_active_reduction_frames"]
    ):
        passed = False
        failed.append("nonstationary:noise_active_reduction")
    if int(delta["noise_active_segment_reduction"]) < int(
        gates["min_nonstationary_noise_segment_reduction"]
    ):
        passed = False
        failed.append("nonstationary:segment_reduction")
    if int(nonstat["candidate_weak_start_blocked_frames"]["total"]) < int(
        gates["min_nonstationary_blocked_weak_start_frames"]
    ):
        passed = False
        failed.append("nonstationary:causal_engagement")

    if not valid:
        decision = "I020_INPUT_INVALID_REVIEW_REQUIRED"
    elif passed:
...
def self_test() -> None:
    ...
    print("I020 weak-start requires-blend evaluator self-test: OK")
