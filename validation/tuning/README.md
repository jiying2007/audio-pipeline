...
That matters because the pairing constraint ties the two axes together. Legalising a ceiling of `C` requires `agc_target_dbfs < C`, so every step of extra ceiling forces the AGC target below it, and the two measured trends work against each other: 2 dB of ceiling buys 0.11 to 0.13 dB of per-case near-end headroom, while the first 2 dB step down in `agc_target_dbfs` costs 0.1867 dB. At the 2 dB granularity every committed space uses, legalising `-18.0` means `agc_target_dbfs=-20.0`, which costs 0.6207 dB against a ceiling gain of roughly 0.48 dB, a net loss of about 0.14 dB. The finer placement that would win, an AGC target just below the ceiling, cannot be written down. `canonical_tuning` requires `agc_target_dbfs < limiter_dbfs` and a cartesian space is materialised as a cross product, so such a grid materialises only when `max(agc_target_dbfs) < min(limiter_dbfs)`. The engine's `validate_search_space` function alone is weaker than that because it probes each axis value against the baseline one at a time; however the workflow gate now calls `generate_candidates` immediately after validation, so a grid that mixes a low ceiling with a higher AGC target is rejected during CI-gate materialisation instead of surviving until tuning runtime. `one-at-a-time` cannot express the coupling at all. The limiter axis is therefore exhausted at `-14.0` for the space format in use, which is a statement about reachability rather than a measured optimum.
