#!/usr/bin/env python3
"""Proxy a frozen processor while canonicalizing C non-finite JSON metrics.

Audio arguments and output are passed through unchanged. Only `--metrics-jsonl`
is redirected to a temporary file so the diagnostic floating-point fields
written by examples/process_pcm.c (`vad_probability`, `erle_db`) can map C
`nan`/`inf` spellings to JSON `null`. Every resulting row is then parsed with
...
def _reject_constant(value: str) -> None:
    raise ValueError(f"non-standard JSON constant remained: {value}")
