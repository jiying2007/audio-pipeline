...
record_pair() {
  mode=$1; frames=$2; base_first=$3
  if [ "$base_first" -eq 1 ]; then
    base_us=$(extract_us "$TMP/base-runtime-bench" "$frames" "$mode"); head_us=$(extract_us "$TMP/head-runtime-bench" "$frames" "$mode")
  else
    head_us=$(extract_us "$TMP/head-runtime-bench" "$frames" "$mode"); base_us=$(extract_us "$TMP/base-runtime-bench" "$frames" "$mode")
  fi
  test -n "$base_us"; test -n "$head_us"
  echo "$base_us" >> "$TMP/base-$mode"; echo "$head_us" >> "$TMP/head-$mode"
  awk -v b="$base_us" -v h="$head_us" 'BEGIN { printf "%.6f\n", 100.0*(h-b)/b }' >> "$TMP/ratio-$mode"
}
i=0
while [ "$i" -lt "$REPS" ]; do
  if [ $((i % 2)) -eq 0 ]; then record_pair minimal "$MINIMAL_FRAMES" 1; record_pair full "$FULL_FRAMES" 1;
  else record_pair minimal "$MINIMAL_FRAMES" 0; record_pair full "$FULL_FRAMES" 0; fi
  i=$((i + 1))
done
