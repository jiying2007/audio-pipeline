...
- `search-spaces/call-agc-limiter-frontier-v1.json` is a bounded paired research path for the lower AGC/limiter frontier. It anchors the already measured `aec_mu=0.24`, `ns_floor=0.07`, `agc_target_dbfs=-16`, `limiter_dbfs=-14` point and then moves AGC and limiter together in six legal 1 dB steps down to `-22/-20` dBFS. It exists to measure a coupled direction directly rather than infer it by adding independent slopes.
