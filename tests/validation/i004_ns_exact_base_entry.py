...
Any unexpected branch file change is fail-closed before execution.
"""
...
def validate_research_scope(base_sha: str) -> tuple[str, list[str]]:
    ...
    require("src/" not in "\n".join(changed) and "CMakeLists.txt" not in changed,
            "shipping DSP/build scope must remain untouched")
...
def self_test() -> None:
    require(all(not path.startswith("src/") for path in ALLOWED_RESEARCH_FILES), "allowed scope contains src")
    ...
    print(json.dumps({"result": "PASS", "authority": "exact-base-entry-only"}, sort_keys=True))
