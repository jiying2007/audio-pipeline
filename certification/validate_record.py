...
POLICY_REQUIRED = {
    "policy_id", "sku", "shipping_approved", "max_active_cpu_percent",
    "max_rss_kib", "max_p95_us", "max_p99_us", "max_soc_c",
    "max_average_power_w", "min_far_end_erle_db", "max_aec_convergence_ms",
    "min_double_talk_near_si_sdr_db", "min_noise_si_sdr_improvement_db",
    "min_vad_f1", "min_soak_hours",
}
BUILD_REQUIRED = {
...
def validate_manifest(manifest: dict, errors: list[str]) -> None:
    ...
    for i, item in enumerate(manifest["artifacts"]):
        if not isinstance(item, dict):
            ...
            continue
        require_keys(item, {"path", "type", "size", "sha256"},
                     f"evidence_manifest.artifacts[{i}]", errors)
...
def validate_build(record: dict, evidence: dict, errors: list[str]) -> None:
    build = record["build"]
    require_keys(build, BUILD_REQUIRED, "build(v4)", errors)
    if errors:
        return
    source_revision = str(build["source_revision"])
    if not HEX40.fullmatch(source_revision):
        errors.append("build.source_revision: v4 requires exact 40-hex Git revision")
    if str(build.get("commit")) != source_revision:
        errors.append("build.commit: must equal build.source_revision")
    if not HEX64.fullmatch(str(build["config_digest"])):
        errors.append("build.config_digest: v4 requires SHA-256")
    if not HEX64.fullmatch(str(build.get("fingerprint", ""))):
        errors.append("build.fingerprint: v4 requires SHA-256")
    for key in ("compiler_id", "compiler_version", "target_triple", "build_type"):
        if not str(build.get(key, "")).strip():
            errors.append(f"build.{key}: v4 requires non-empty value")
    binaries = build.get("binary_sha256")
    if not isinstance(binaries, dict) or not binaries:
        errors.append("build.binary_sha256: v4 requires at least one binary")
        return
...
def validate_deployment(record: dict, evidence: dict, errors: list[str]) -> None:
    deployment = record.get("deployment")
    if not isinstance(deployment, dict):
        errors.append("deployment(v4): required object")
        return
    require_keys(deployment, DEPLOYMENT_REQUIRED, "deployment(v4)", errors)
    if errors:
        return
    ...
    if not isinstance(toolchain, dict):
        ...
        require_keys(toolchain, TOOLCHAIN_REQUIRED, "deployment.toolchain", errors)
        ...
        if not isinstance(cmake_args, list) or not cmake_args or not all(
            isinstance(item, str) and item for item in cmake_args
        ):
...
def validate(
    ...
    require_keys(
        record,
        {"schema_version", "sku", "status", "build", "platform", "audio_route",
    ...
    if record.get("schema_version") != 4:
        errors.append("schema_version: v2 hard cut accepts schema v4 only")
        return errors
    status = record["status"]
    if status not in {"pending", "board-validated", "product-certified", "failed"}:
        errors.append(f"status: unsupported value {status!r}")
    require_keys(record["build"], {"commit", "version", "fingerprint", "compiler", "abi"}, "build", errors)
    require_keys(record["platform"], {"soc", "kernel", "governor", "cpuset"}, "platform", errors)
    require_keys(record["audio_route"], {"capture_device", "playback_device", "sample_rate_hz", "mic_channels"}, "audio_route", errors)
    ...
    require_keys(
        record,
        {"policy", "policy_sha256", "corpus_manifest_sha256",
         "evidence_manifest_sha256", "collector_version", "toolchain_digest",
         "thermal_power", "deployment"},
        "record", errors,
    )
    ...
    if "not-for-shipping" in policy_id or policy_id.startswith("example-"):
    ...
    require_keys(soak, {"hours", "passed"} | SOAK_REQUIRED, "soak(v4)", errors)
    require_keys(
        artifacts,
        {"result_json", "benchmark_json", "evidence_manifest", "sha256",
         "binary_sha256", "deployment_provenance"},
        "artifacts(v4)", errors,
    )
    if errors:
        return errors
    ...
    for key in {"xruns", "deadline_misses", "overruns", "input_full_events", "output_drop_events", "failed_frames"}:
        if int(soak[key]) != 0:
            errors.append(f"soak.{key}: v4 nominal gate requires 0")
...
def self_test() -> None:
    ...
    policy = {
        "policy_id": "test-policy", "sku": "test", "shipping_approved": True,
        "max_active_cpu_percent": 40, "max_rss_kib": 4096,
        "max_p95_us": 7000, "max_p99_us": 9000, "max_soc_c": 85,
        "max_average_power_w": 2, "min_far_end_erle_db": 15,
    ...
    with tempfile.TemporaryDirectory(prefix="ap-cert-validator-") as temporary:
        ...
        evidence = {
            "schema_version": 1, "collector_version": "4", "generated_at": "now",
            "artifacts": [
        ...
        record = {
            ...
            "build": {
                "commit": "a" * 40, "version": "2.0.0", "fingerprint": "6" * 64,
                "compiler": "gcc", "abi": "armv7", "source_revision": "a" * 40,
                "config_digest": "7" * 64, "compiler_id": "GNU", "compiler_version": "13.3",
            ...
            "deployment": {
                ...
                "builder_runner": "builder-1", "dut_runner": "dut-1",
                "binary_sha256": {"ap_bench": binary_hash}, "result": "PASS",
                "toolchain": {
                    "compiler_path": "/opt/tc/bin/cc", "compiler_sha256": "8" * 64,
                    "compiler_version": "GNU 13.3", "sysroot_path": "/opt/tc/sysroot",
                    "sysroot_sha256": "9" * 64, "toolchain_root_path": "/opt/tc",
                    "toolchain_root_sha256": "a" * 64, "cflags": "-O2 -mcpu=cortex-a32",
                    "cmake_args": cmake_args, "cmake_args_sha256": cmake_args_hash,
                },
            },
            "platform": {"soc": "test", "kernel": "6.6", "governor": "performance", "cpuset": "1"},
            "audio_route": {"capture_device": "hw:0,0", "playback_device": "hw:0,0", "sample_rate_hz": 16000, "mic_channels": 2},
            "performance": {"active_cpu_percent": 20, "p95_us": 3000, "p99_us": 5000, "deadline_misses": 0, "rss_kib": 512, "xruns": 0, "overruns": 0, "input_full_events": 0, "output_drop_events": 0},
            "acoustic": {"corpus_revision": "r1", "cases_total": 10, "cases_passed": 10, "far_end_erle_db": 20, "aec_convergence_ms": 500, "double_talk_near_si_sdr_db": 8, "noise_si_sdr_improvement_db": 4, "vad_f1": 0.9, "threshold_report": "result.json"},
            "thermal_power": {"ambient_c": 25, "max_soc_c": 60, "average_power_w": 1},
            "soak": {"hours": 72, "passed": True, "xruns": 0, "deadline_misses": 0, "overruns": 0, "input_full_events": 0, "output_drop_events": 0, "failed_frames": 0, "p95_us": 1000, "p99_us": 2000},
            "artifacts": {"result_json": "result.json", "benchmark_json": "bench.json", "evidence_manifest": "evidence.json", "deployment_provenance": "deployment-provenance.json", "binary_sha256": {"ap_bench": binary_hash}, "sha256": "0" * 64},
        }
        assert validate(record, policy, ph, evidence, eh, ch, root) == []
        legacy = json.loads(json.dumps(record))
        legacy["schema_version"] = 3
        assert any("v4 only" in error for error in validate(legacy, policy, ph, evidence, eh, ch, root))
...
def main() -> int:
    ...
    evidence = json.loads(args.evidence_manifest.read_text(encoding="utf-8")) if args.evidence_manifest else None
    errors = validate(
        record, policy, sha256(args.policy) if args.policy else None,
        evidence, sha256(args.evidence_manifest) if args.evidence_manifest else None,
