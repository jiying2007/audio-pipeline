...
# Compare base and candidate on the same runner/compiler. Using the median of
# alternating runs reduces hosted-runner scheduling, thermal and frequency noise.
git fetch origin main --depth=1
git worktree add --detach "$TMP/base" "$BASE_REF" >/dev/null

COMMON_FLAGS='-DCMAKE_BUILD_TYPE=Release -DAP_BUILD_TESTS=OFF -DAP_BUILD_EXAMPLES=OFF -DAP_ENABLE_RUNTIME=OFF'
...
i=0
while [ "$i" -lt "$REPS" ]; do
  # Alternate order every repetition to avoid consistently favoring one build.
  if [ $((i % 2)) -eq 0 ]; then
...
BASE_ACTIVE=$(median "$TMP/base-active")
HEAD_ACTIVE=$(median "$TMP/head-active")
BASE_IDLE=$(median "$TMP/base-idle")
HEAD_IDLE=$(median "$TMP/head-idle")
...
# Hosted hardware still varies between workflow runs, but base and head share
# this runner. Fail only on a clear same-runner regression; small deltas remain
# diagnostic rather than release claims.
if awk -v d="$ACTIVE_DELTA" 'BEGIN { exit !(d > 10.0) }'; then
  echo "active same-runner regression exceeds 10%" >&2
  exit 3
fi
if awk -v d="$IDLE_DELTA" 'BEGIN { exit !(d > 10.0) }'; then
  echo "idle same-runner regression exceeds 10%" >&2
  exit 4
fi
