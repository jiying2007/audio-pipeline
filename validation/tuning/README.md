...
A legal finer pair can be written: tuning values are arbitrary floats, and a Cartesian space is valid whenever every generated cross-product satisfies `agc_target_dbfs < limiter_dbfs`. What the current rectangular Cartesian strategy cannot express compactly is a diagonal list of several correlated AGC/limiter pairs without also generating their cross-pairs; `one-at-a-time` cannot express that coupling at all. The engine's `validate_search_space` function alone still probes axes against the baseline one at a time, but the workflow gate now immediately calls `generate_candidates`, so any illegal rectangular cross-product fails during CI-gate materialisation rather than at tuning runtime. The committed limiter grid is therefore exhausted at `-14.0`; the lower **paired parameter frontier is not**. It remains unmeasured and must not be described as rejected, optimal or unreachable until a direct legal coupled experiment is run.
