...
CASES = {
    "capture-gap": {"flag": "capture_discontinuity", "family": "capture-io", "hypothesis": "capture-io"},
    "render-gap": {"flag": "render_discontinuity", "family": "sync-reference", "hypothesis": "sync-reference-path"},
    "clock-reset": {"flag": "clock_reset", "family": "sync-reference", "hypothesis": "sync-reference-path"},
    "xrun": {"flag": "xrun", "family": "runtime-continuity", "hypothesis": "runtime-continuity"},
    "codec-reopen": {"flag": "codec_reopen", "family": "capture-io", "hypothesis": "capture-io"},
}
...
def main() -> int:
    ...
    for case, expected in CASES.items():
        ...
        triage_command = [
            sys.executable, str(triage_tool), str(dump),
            "--processor", str(args.processor),
            "--output-dir", str(triage_dir),
        ]
        if args.processor_build_info:
            triage_command.extend([
                "--processor-build-info", str(args.processor_build_info),
                "--require-build-identity-match",
            ])
        run(triage_command)
        run([
            sys.executable, str(diagnose_tool), str(triage_dir / "triage.json"),
        ...
        metadata_anomalies = [item for item in analysis["anomalies"] if item.get("kind") == "metadata"]
        ...
        assert frames_before == 2 and frames_after == 2, (case, frames_before, frames_after)
        ...
        assert has_metadata_evidence(expected_hypothesis, expected["flag"], EXPECTED_FAULT_FRAME), (case, expected_hypothesis)
        ...
        assert replay_authority.get("authority") == "repository-internal-replay-interpretation-only", (case, replay_authority)
        ...
        assert replay_authority.get("runtime_metadata_state_present") is True, (case, replay_authority)
        assert set(replay_authority.get("runtime_metadata_flags") or []) == {expected["flag"]}, (case, replay_authority)
        assert replay_authority.get("comparison_present") is True, (case, replay_authority)
        assert replay_authority.get("bit_exact") == comparison.get("bit_exact"), (case, replay_authority, comparison)
        assert replay_authority.get("classification") == "stateful-runtime-context-not-replayed", (case, replay_authority)
        assert replay_authority.get("whole_incident_equivalence_authoritative") is False, (case, replay_authority)
        assert "not re-injected" in str(replay_authority.get("bit_exact_claim_scope")), (case, replay_authority)

        build_identity = triage.get("build_identity") or {}
        if args.processor_build_info:
            assert build_identity.get("authority") == "repository-internal-build-identity-subset-only", (case, build_identity)
            assert build_identity.get("status") == "MATCH", (case, build_identity)
            assert build_identity.get("shared_fields_match") is True, (case, build_identity)
            ...
            assert build_identity.get("exact_source_config_match_authoritative") is False, (case, build_identity)

        summary["cases"].append({
            "case": case,
            "metadata_flag": expected["flag"],
            "recording_trigger": {"event": trigger.get("event"), "name": trigger.get("name"), "relation": trigger.get("relation")},
            "incident_window": {
                "recorded_frames": 5,
                "sequences": sequences,
            ...
            "temporal_first_fault": {"frame": first.get("frame"), "kind": first.get("kind"), "family": first.get("family")},
            ...
            "replay_authority": {
                ...
                "runtime_metadata_state_present": replay_authority.get("runtime_metadata_state_present"),
                "runtime_metadata_flags": replay_authority.get("runtime_metadata_flags"),
                "classification": replay_authority.get("classification"),
                "whole_incident_equivalence_authoritative": replay_authority.get("whole_incident_equivalence_authoritative"),
            },
            "build_identity": {
                "status": build_identity.get("status"),
                "shared_fields_match": build_identity.get("shared_fields_match"),
                "exact_source_config_match_authoritative": build_identity.get("exact_source_config_match_authoritative"),
            },
            "causal_proof": False,
        })
