...
Every normal triage invocation requires processor build information generated by compiling `tests/diagnostics/build_identity_probe.c` against the exact processor library under test. `Diagnostic Triage` CI performs that compilation explicitly and writes `processor-build-info.json`. Given that JSON:
