...
COMMON_FLAGS='-DCMAKE_BUILD_TYPE=Release -DAP_BUILD_TESTS=OFF -DAP_BUILD_BENCH=OFF -DAP_BUILD_EXAMPLES=OFF -DAP_ENABLE_RUNTIME=OFF'
...
compile_harness() {
  ...
  ${CC:-cc} -O3 -std=c11 \
    -I"$ROOT/include" \
    "$ROOT/bench/bench_resampler_path.c" \
    "$build_dir/libaudio_pipeline.a" -lm -o "$output"
}

compile_harness "$TMP/base/build-resampler-perf" "$TMP/base-resampler-bench"
compile_harness "$TMP/head-build" "$TMP/head-resampler-bench"

extract_us() {
  "$1" "$2" "$3" "$FRAMES" | sed -n 's/.* us_per_frame=\([0-9.]*\).*/\1/p'
}
...
run_case() {
  ...
  while [ "$i" -lt "$REPS" ]; do
    if [ $((i % 2)) -eq 0 ]; then
      base_us=$(extract_us "$TMP/base-resampler-bench" "$io" "$internal")
      head_us=$(extract_us "$TMP/head-resampler-bench" "$io" "$internal")
    else
      head_us=$(extract_us "$TMP/head-resampler-bench" "$io" "$internal")
      base_us=$(extract_us "$TMP/base-resampler-bench" "$io" "$internal")
    fi
    test -n "$base_us"
    test -n "$head_us"
    echo "$base_us" >> "$TMP/base-$label"
    echo "$head_us" >> "$TMP/head-$label"
  ...
  if awk -v d="$delta" 'BEGIN { exit !(d > 10.0) }'; then
    echo "resampler same-runner regression exceeds 10% io=$io internal=$internal" >&2
    exit 3
  fi
}
