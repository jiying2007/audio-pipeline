...
static float normalized_excess_concentration(double sum,
                                             double square_sum,
                                             uint32_t bins) {
    if (sum <= 1.0e-12 || bins == 0u) return 0.0f;
    return (float)((double)bins * square_sum / (sum * sum + 1.0e-18));
}
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        double base_excess_sum = 0.0;
        double base_excess_sq_sum = 0.0;
        double stress_excess_sum = 0.0;
        double stress_excess_sq_sum = 0.0;
        uint32_t band_bins = 0u;
        uint32_t i;
        uint32_t k;
        float base_snr_db;
        float stress_snr_db;
        float base_concentration;
        float stress_concentration;
        ...
        for (k = band_lo + 1u; k < band_hi; ++k) {
            ...
            const double post_base = power / nb;
            const double post_stress = power / ns;
            const double excess_base = post_base > 1.0 ? post_base - 1.0 : 0.0;
            const double excess_stress =
                post_stress > 1.0 ? post_stress - 1.0 : 0.0;

            power_sum += power;
            base_noise_sum += nb;
            stress_noise_sum += ns;
            base_excess_sum += excess_base;
            base_excess_sq_sum += excess_base * excess_base;
            stress_excess_sum += excess_stress;
            stress_excess_sq_sum += excess_stress * excess_stress;
            band_bins++;
        }
        if (band_bins == 0u) {
            fclose(input_file);
        ...
        base_concentration = normalized_excess_concentration(
            base_excess_sum, base_excess_sq_sum, band_bins);
        stress_concentration = normalized_excess_concentration(
            stress_excess_sum, stress_excess_sq_sum, band_bins);

        printf(
            "{\"frame\":%u,"
            "\"concentration_base\":%.9g,"
            "\"concentration_stress\":%.9g,"
            "\"spectral_base_snr_db\":%.9g,"
