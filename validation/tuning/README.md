...
The validation step in `.github/workflows/acoustic-tuning-iteration.yml` discovers every file under `search-spaces/`, runs the engine's `validate_search_space`, and then materialises each space with `generate_candidates`. A space that nobody remembered to enumerate is therefore still checked instead of silently skipped, cross-product and paired tuning constraints plus each space's `max_candidates` are exercised at the gate, the CALL spaces remain pinned to their expected strategy, and an unregistered `call-*` space fails closed.
...
That is also the reason to treat it carefully. Lowering the output ceiling is a product-level perceptual change. The objective has no graded output-level term; the only output-level constraint anywhere in the pipeline is a per-case pass/fail floor, `min_output_rms_dbfs`, which is `-50.0` dBFS on the speech cases and `-100.0` to `-110.0` dBFS on the far-end cases where the near end is silent. Measured on `clean-capture` at `aec_mu=0.24`, `ns_floor=0.07`, `agc_target_dbfs=-16.0`, the ceiling is the output peak exactly (`-2.0`, `-6.0`, `-10.0`, `-12.0` and `-14.0` dBFS each come back as the same peak in dBFS), while output RMS falls from `-19.01` dBFS at the shipping `-2.0` ceiling to `-20.65` dBFS at `-10.0`, `-22.12` dBFS at `-12.0` and `-23.81` dBFS at `-14.0`. The champion's ceiling therefore costs about 1.6 dB of output RMS, and it still sits 29.4 dB above the `-50.0` dBFS floor, so the floor is not what bounds the direction. What actually bounds it is the pairing constraint: `agc_target_dbfs < limiter_dbfs` is enforced by the engine's canonical tuning contract when concrete candidates are materialised and by the processor itself. The CI gate now materialises every committed search space as well, so an invalid cross-product fails before tuning execution. A candidate whose `agc_target_dbfs` is `-16.0` therefore cannot use a ceiling at or below `-16.0` at all, and reaching a lower ceiling means lowering the AGC target, which is itself gate-bound. The limiter does not act in the `ns-isolated` profile, where the output was identical at every ceiling tested, so this cost is confined to the `default` profile. The engine and the processor agree exactly on the accepted ranges: `limiter_dbfs` in `[-20.0, -0.1]`, `agc_target_dbfs` in `[-60.0, -1.0]`, `ns_floor` in `[0.02, 1.0]` and `aec_mu` in `(0, 1]`. `search-spaces/call-interaction-3d-v1.json` covers this region as a research space; its results are discovery input only, and the blind holdout plus product review remain mandatory.
...
The same space also sweeps `ns_floor` at 0.06 and 0.07, and the second value never wins: `ns_floor=0.07` outscores 0.06 in all nine cells. More usefully, the champion's near-end margin does not survive a step in `ns_floor`. At `limiter_dbfs=-10.0` and `aec_mu=0.24` the worst per-case near-end delta is -0.7447 dB at `ns_floor=0.07`, which is 0.0053 dB inside the -0.75 dB bound, and -0.7884 dB at `ns_floor=0.06`, which violates it. The 0.0053 dB figure is therefore not merely small: it is small enough that the adjacent grid point in a different control fails the gate. `aec_mu=0.26` is admissible only at `limiter_dbfs=-14.0`, at both `ns_floor` values, so its admissibility is set by the ceiling and not by `ns_floor`.

Re-probing the ceiling at 1 dB granularity makes that trade exact, and corrects an earlier reading of it. The grid above uses 2 dB steps, so it only shows which of its own points survive the `ns_floor` step. Probed at -10.0, -11.0, -12.0, -13.0 and -14.0 with `aec_mu=0.24` and `agc_target_dbfs=-16.0`, at both `ns_floor` values, the worst per-case near-end SI-SDR delta is:

```
limiter_dbfs    -10.0     -11.0     -12.0     -13.0     -14.0
ns_floor=0.07    0.0053    0.0480    0.1185    0.1929    0.2457
ns_floor=0.06   -0.0384    0.0037    0.0728    0.1452    0.1953
score            4.5914    4.5525    4.5245    4.4630    4.4409
```

The step in `ns_floor` stops violating between -10.0 and -11.0, not between -11.0 and -12.0, so -12.0 is not the only ceiling that survives it. What separates the two is the size of the worst case: at -11.0 it is 0.0037 dB, thinner than the champion's own 0.0053 dB, so -11.0 is the boundary and not a robustness gain. Read as score spent per dB of worst-case margin bought, cumulative from the champion, the exchange rate is 0.926 at -11.0, 0.602 at -12.0, 0.699 at -13.0 and 0.644 at -14.0, so -12.0 is the cheapest point on the curve: -11.0 buys 38% of the available margin for 58% of the available score, and -13.0 is interior for the same reason. The champion's compliance is therefore local to its own grid point. Its immediate neighbour in `ns_floor` fails the case-delta gate, and the first ceiling whose worst case is wider than the champion's own is -12.0.
...
`agc_target_dbfs` below the champion is the same shape: it spends the binding margin rather than buying it. Probed one at a time against the champion (`ns_floor=0.07`, `aec_mu=0.24`, `agc_target_dbfs=-16.0`, `limiter_dbfs=-10.0`), the engine reports `KEEP_BASELINE`, but the score is not the reason:

```
agc_target_dbfs     score    worst per-case near-end delta
-16.0 (champion)   0.0000     0.0000
-18.0             +0.0841    -0.1867
-20.0             -0.3247    -0.6207
-22.0             -1.1508    -1.1854  violates
```

The score is non-monotone, `-18.0` sits 0.0841 above the champion, which is under the space's 0.10 minimum improvement. The per-case near-end delta is not non-monotone: it falls monotonically and accelerates, `-0.1867`, `-0.6207` and `-1.1854` dB for 2, 4 and 6 dB of AGC. The cheapest step down therefore costs 0.19 dB of the one bound the champion is closest to, in exchange for a score move below the improvement threshold.

That matters because the pairing constraint ties the two axes together. Legalising a ceiling of `C` requires `agc_target_dbfs < C`, so the isolated limiter and AGC probes point in opposite directions: 2 dB of lower ceiling bought roughly 0.11 to 0.13 dB of per-case near-end headroom in the measured slice, while the first 2 dB step down in `agc_target_dbfs` cost 0.1867 dB. At the 2 dB granularity used by the committed grids, a `limiter_dbfs=-18.0` point would pair naturally with an AGC target below it such as `-20.0`; the isolated numbers suggest that trade may be unfavourable, but they are not a coupled measurement and must not be added as if the interaction were linear. Earlier CALL experiments already show that these controls interact strongly, so independent slopes are evidence for where to probe, not evidence that the paired frontier is closed.

The engine keeps the fail-closed `paired` strategy as a general capability for correlated controls: configured parameter lists are zipped by index, must have equal lengths, and every complete tuple is checked by the canonical tuning contract before it can become a candidate. Adjacent-sensitivity evidence follows the paired path as previous/next already-evaluated tuples instead of inventing independent one-axis neighbors.

The lower AGC/limiter paired direction is now closed as a **generic CALL optimization direction**. Acoustic Tuning run `35568818095` on source `69e973f17184a771dee7929d39d98c0bbde86a0d` evaluated the seven-point path from the `-16/-14` research anchor down to `-22/-20` dBFS. The generic regression objective selected `agc_target_dbfs=-20`, `limiter_dbfs=-18` with development score `+0.5265` and emitted `ACOUSTIC_CANDIDATE`; all six lower pairs were development-positive. That result was not sufficient evidence because the generic objective does not grade near-end target level.

The exact same seven-point path was therefore replayed through the scale-sensitive Audio Quality layer in run `35571014346` on source `288ad1199168eecc808b059ea02f17aee05a121c`. With the existing `p10_near_projection_gain_db` objective and no newly invented threshold, the decision became `KEEP_BASELINE`. Development scores were `0.0000, -0.1961, -0.4724, -0.6939, -1.0230, -1.3599, -1.8161` from `-16/-14` through `-22/-20`; the generic winner `-20/-18` was therefore quality-negative. The corresponding p10 near-projection gain moved monotonically from `-4.8862` dB at the anchor to `-5.0262, -5.2236, -5.3818, -5.5711, -5.8118, -6.1376` dB. Other principal quality metrics were effectively unchanged over the path, isolating target-level loss as the reason the generic score was misleading.

Accordingly, the generic `call-agc-limiter-frontier-v1.json` execution surface is retired rather than left available to reproduce a known quality-invalid candidate. AGC/limiter level-changing research now belongs in Audio Quality Evaluation, where target-level preservation is explicit. This does not retire `paired` strategy itself and does not change shipping defaults.

The default CALL searches start from the shipping CALL defaults. Correlated research paths may still use the paired strategy when their quality authority is appropriate. All of these spaces use the controls exposed by `ap_tuning_t`:
...
One-at-a-time is intentional for the default CI loop: it keeps cost bounded, provides causal attribution for a gain/regression, and avoids blindly exploring an exponential grid. The engine also supports capped Cartesian search for explicit interaction studies and paired search for correlated parameter paths whose legal tuples must move together.
