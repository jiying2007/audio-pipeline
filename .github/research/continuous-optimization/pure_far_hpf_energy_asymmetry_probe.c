...
int main(int argc, char **argv) {
    ...
    while (fread(mic, sizeof(int16_t), frame, fm) == frame) {
        ...
        shadow_hpf_ratio = shadow_hpf_energy / (shadow_raw_energy + 1.0e-12f);
        shadow_hpf_delta_db = 10.0f * log10f(
            (shadow_hpf_energy + 1.0e-12f) / (shadow_raw_energy + 1.0e-12f));
