#!/usr/bin/env python3
"""Exercise the active I020 one-shot guard without running fresh research."""
...
class GuardTests(unittest.TestCase):
    def execute(self,*,runs=None,jobs=None,attempt="1",api_error=False):
        ...
        def api(args,**kwargs):
            ...
            if "/workflows/" in endpoint:
                ...
                pages=jobs.get(
                    (int(match.group(1)),int(match.group(2))),[{"jobs":[]}])
    ...
    def test_pre_fresh_failure_allows_new_dispatch(self):
        self.execute(**self.history([
            ...
            step(GENERATE,"skipped"),step(EVALUATE,"skipped")]))
    ...
    def test_guard_precedes_fresh_steps(self):
        text=WORKFLOW.read_text()
        guard=text.index("      - name: Enforce one-shot I020 development execution\n")
