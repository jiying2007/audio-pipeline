...
int main(int argc, char **argv) {
    ...
    for (f = 0; f < frames; ++f) {
        ...
        for (i = 0; i < 160u; ++i) {
            ...
            const float far = idle_mode ? 0.0f :
                              0.18f * sinf(2.0f * PI_F * 733.0f * (float)s / 16000.0f);
    ...
    ap_pipeline_get_metrics(p, &m);
    {
        ...
        printf("mode=%s frames=%u audio_s=%u elapsed_s=%.6f avg_us=%.2f p50_us=%u p95_us=%u p99_us=%u max_us=%u deadline_misses=%u rtf=%.5f state_bytes=%zu aec_backend=%d aec_block=%u aec_partitions=%u aec_taps=%u erle_db=%.2f delay_ms=%u delay_error=%d drift_ppm=%.2f slips=%llu delay_jumps=%llu aec_resets=%llu res_gain=%.3f freq_res=%u quality=%d\n",
               idle_mode ? "idle" : "active", frames, seconds, elapsed_s,
               us_frame_avg, p50, p95, p99, max_us, deadline_misses, rtf,
               ap_pipeline_state_size(), (int)m.aec_backend, m.aec_block_samples,
               m.active_aec_partitions, m.active_aec_taps, m.erle_db,
               m.estimated_delay_ms, (int)m.delay_error_samples,
